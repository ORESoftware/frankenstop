...
function validate(schema, propertiesList, model) {
  ...
  const errors = [];
  const forceAllObjectsToHavePropertyValidation = Boolean(schema.forceAllObjectsToHavePropertyValidation);
  
  //TODO: validate that no extraneous properties exist
  const allowExtraneousProperties = Boolean(schema.allowExtraneousProperties);
      ...
      Object.keys(m).forEach(function (k) {
        ...
        try {
          ...
          if (typeof  subModel === 'object' && subModel !== null) {
            ...
            let hasProperties = typeof subSchema.properties === 'object';
            let hasValues = typeof subSchema.values === 'object';
            let hasElements = typeof subSchema.elements === 'object';
            ...
            if ([hasProperties, hasValues, hasElements].filter(i => i).length > 1) {
              ...
              if (subModel) {
                assert(Array.isArray(subModel), 'In this case, subModel should be an "array" type.');
                
                let type = subSchema.elements.type || 'object';
                
                subModel.forEach(function (m) {
                  assert.equal(typeof m, type);
                  if (type === 'object') {
  ...
  (function recurseThroughProperties(schemaProperties, model, prevSchema, prevModel, prevKey) {
    ...
    Object.keys(schemaProperties).forEach(function (key, index) {
      ...
      if (!model) {
        throw new Error('Missing model object.');
      }
      
      const modelProp = model[key];
      const isRequired = Boolean(schemaProp.required);
